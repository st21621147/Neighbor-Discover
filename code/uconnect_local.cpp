...
void init()
{
	...
	stringstream ss;
	ss<<"uconnect_local_neighbor.txt";
...
void uconnect_local(int timeInterval)
{
	init();
	...
	for(int time = 0; time < timeInterval; time++){
		...
		for(int i = 0; i < N; i++){
			if(nodelist[i].isOff(time) == false){
				for(int j = 0; j < N; j++){
					if(neighborMap[i][j] == 1 && nodelist[j].isOff(time) == false){
						onNeighbor[i]++;
					}
				}
			}
		...
		for(int i = 0; i < N; i++){
			if(nodelist[i].isOff(time) == false && onNeighbor[i] == 1){
				for(int j = 0; j < N; j++){
					if(neighborMap[i][j] == 1 && nodelist[j].isOff(time) == false){
						setLatency(i, j, time);
						break;
	...
	stringstream sss;
	sss<<"uconnect_local_latency.txt";
