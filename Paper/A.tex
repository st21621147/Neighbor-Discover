...
In large-scale networks, nodes are not fully-connected and thus communication may fail due to concurrent transmissions. 
...
In a large-scale network, we ignore the boundary area of the network.  %Atend assume nodes in the network are of an enormous quantity. 
Note that, when the network covered area $D$ is much larger than the area $R_i$ of node $u_i$, we have:
\begin{equation}
\label{eqnNB}
...
\STATE Set tranmsit probability $p_t^i := \frac{1}{\widetilde{n_i}+1}$, $t := 0$;  \label{pro}
...
\label{uniform}
%background
Uniform distribution is a basically used in deployment of wireless networks.
