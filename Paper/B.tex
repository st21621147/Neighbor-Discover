...
the fixed pointer of node $i$ in each period $T_i$ and the fixed pointer of 
node $j$ in each period $T_j$ will rendezvous in every $T_iT_j$  according 
to Chinese remainder theorem.

\textbf{Case 2: $T_i = T_j$}. Since $T_i = T_j$, the $k_i^{th}$ time slot in $T_i$ will
always rendezvous with the $k_j^{th}$ time slot in $T_j$.
