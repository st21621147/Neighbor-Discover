...
By experiment, we find collisions caused by simultaneous transmissions result in the waste of time and energy. We consider a large-scale network with 2000 nodes obeying a uniform distribution. Collisions of searchlight \cite{bakht2012searchlight}, the optimal deterministic algorithm, happen as frequently as 99.8\% of the time; collisions of PND \cite{song2014probabilistic}, a typical probabilistic algorithm, happen as frequently as 43.4\% of the time. Aloha-like \cite{you2011aloha}, the optimal probabilistic algorithm, shows a high idle rate (none of the neighbors are transmitting) as 94.6\% of the time, which reduces the collision excessively and still results in a high latency. This is the same with CSMA \cite{XX}, a general collision avoidance  technique in networks, the idle rate of which is XX\%. That means, because of collisions, existing works waste time and energy, and cannot achieve low latency and energy efficiency for large-scale networks.
